...
const TRADE_WHISPER = {
  ...
  'zh_CN': /^你好，我想用 (?<price>.+) 购买 (?<item>.+) 在 (?<league>.+) \(仓库页 "(?<tab_name>.*)"; 位置: 左 (?<tab_left>\d+), 上 (?<tab_top>\d+)\)(?<message>.+)?$/
}
...
const TRADE_BULK_WHISPER = {
  ...
  'cmn-Hant': /^你好，我想用 (?<price>.+) 購買 (?<item>.+) 在 (?<league>.+)\.(?<message>.+)?$/,
  'zh_CN': /^你好，我想用 (?<price>.+) 购买 (?<item>.+) 在 (?<league>.+)\.(?<message>.+)?$/
}
